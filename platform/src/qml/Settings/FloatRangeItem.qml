...
Item {
    ...
    property int precision: (''+item_data.step).split('.')[1].length
    property int max_symbols: (''+item_data.max).split('.')[0].length+precision+1
    ...
    Slider {
        ...
        onPressedChanged: {
            if( !pressed ) {
                ...
                cfg.setVal(key_path, Number.parseFloat(value))
            }
        }
    }
    ...
    Rectangle {
        ...
        Text {
            ...
            text: Number.parseFloat(slider.value).toFixed(precision).padStart(max_symbols, '0');
